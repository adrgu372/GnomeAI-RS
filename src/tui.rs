...
const COMMANDS: &[(&str, &str)] = &[
    ...
    ("/memory", "show shared memory; /memory clear|on|off"),
...
async fn run_account_login(flow: AccountLogin) -> Result<()> {
    match flow {
        ...
        AccountLogin::ClaudeCode => {
            const PROGRAM: &str = "claude";
            const ARGS: &[&str] = &["auth", "login"];
            ...
            let status = tokio::process::Command::new(PROGRAM)
                ...
                .map_err(|error| {
                    anyhow::anyhow!(
                        "cannot start `{PROGRAM}` ({error}); install the official CLI first"
                    )
                })?;
            if !status.success() {
...
fn set_clipboard_notice(app: &mut App, result: std::io::Result<()>, characters: usize) {
...
async fn submit(app: &mut App, text: String, ops: &mpsc::Sender<Op>) {
    ...
    match trimmed {
        ...
        "/sandbox" => {
            app.blocks.push(Block_::Error(
                "usage: /sandbox read-only|workspace-write|danger-full-access".into(),
            ));
            return;
        }
        _ => {}
    ...
    if let Some(argument) = trimmed.strip_prefix("/memory ") {
        match argument.trim().to_ascii_lowercase().as_str() {
            ...
            _ => app
                .blocks
                .push(Block_::Error("usage: /memory [show|clear|on|off]".into())),
        }
        return;
    }
...
fn draw_completions(f: &mut Frame, app: &App, composer: Rect) {
    let h = (app.completions.len() as u16).min(6) + 2;
    let area = Rect {
        x: composer.x,
        y: composer.y.saturating_sub(h),
        width: composer.width.min(50),
        height: h,
    };
    ...
    let items: Vec<ListItem> = app
        ...
        .map(|(i, name)| {
            let desc = COMMANDS
                ...
                .map(|(_, d)| *d)
                .unwrap_or("");
            let style = if i == app.completion_idx {
                Style::new().bg(Color::DarkGray).bold()
            } else {
                Style::new()
            };
            ListItem::new(Line::styled(format!("{name}  {desc}"), style))
        })
        .collect();
