...
pub enum ApprovalPolicy {
    /// Ask for everything that is not read-only.
    Untrusted,
    /// Ask only when the sandbox refused and we would need to widen it.
    OnFailure,
    /// The model asks when it thinks it needs to; otherwise run.
    OnRequest,
    Never,
}
...
impl Agent {
    ...
    async fn approved(&self, call: &ToolCall, tool: &Arc<dyn Tool>, summary: &str) -> Result<bool> {
        ...
        if self.approval != ApprovalPolicy::Untrusted {
            return Ok(true);
        }
        ...
        let _ = self
            ...
            .send(Event::ApprovalRequest {
                ...
                reason: format!("`{}` writes or executes", call.name),
            })
            .await;
        ...
        while let Some((id, decision)) = rx.recv().await {
            ...
            return Ok(match decision {
                ...
                Decision::AlwaysAllow => {
                    self.always_allow.lock().await.push(summary.to_string());
                    true
                }
                Decision::Deny => false,
            });
