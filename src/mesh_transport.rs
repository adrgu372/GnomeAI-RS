...
impl Mesh {
    pub fn new(home: PathBuf) -> Self {
        ...
        let state = Arc::new(Mutex::new(
            json!({"status":"stopped","socks_port":0,"onions":{}}),
        ));
    ...
    pub fn request(&self, action: &str, payload: &Value) -> Result<Value> {
        if action == "mesh_start" {
            ...
            {
                bail!("Invalid mesh channel");
            }
            let port = payload["port"]
                .as_u64()
                .filter(|p| *p > 0 && *p <= 65535)
                .context("Local peer port required")? as u16;
            self.tx
                .try_send(Command::Start {
                    channel: channel.into(),
                    port,
                })
                .context("Tor request queue is busy")?;
        } else if action == "mesh_revoke" {
            self.tx.try_send(Command::Revoke(
                payload["channel"]
                    .as_str()
...
async fn run(
    ...
    let slots = budget.clone();
    tokio::spawn(async move {
        loop {
            let incoming = tokio::select! {_=proxy_stop.cancelled()=>break,s=socks.accept()=>s};
            let Ok((mut local, _)) = incoming else {
                break;
            };
            let Ok(permit) = slots.clone().try_acquire_owned() else {
                continue;
            };
            let client = dialer.isolated_client();
            let cancel = proxy_stop.clone();
            tokio::spawn(async move {
                let _permit = permit;
                let task = async {
                    ...
                    if greeting != [5, 1, 0] {
                        bail!("SOCKS authentication method unsupported");
                    }
                    local.write_all(&[5, 0]).await?;
                    let mut header = [0; 5];
                    local.read_exact(&mut header).await?;
                    if header[..4] != [5, 1, 0, 3] {
                        bail!("Only onion hostnames are accepted");
                    }
                    let mut host = vec![0; header[4] as usize];
                    local.read_exact(&mut host).await?;
                    let port = local.read_u16().await?;
                    let host = std::str::from_utf8(&host)?;
                    if !valid_onion(host) || port != 80 {
                        bail!("Only GnomeAI onion endpoints are accepted");
                    }
                ...
                if let Ok(Ok(remote)) = setup {
                    let mut remote = remote.compat();
                    tokio::select! {_=cancel.cancelled()=>{},_=tokio::io::copy_bidirectional(&mut local,&mut remote)=>{}}
                }
            });
        }
    });
    ...
    loop {
        let cmd = if let Some(cmd) = next.take() {
            cmd
        } else {
            tokio::select! {_=stop.cancelled()=>break,cmd=rx.recv()=>{let Some(cmd)=cmd else{break;};cmd}}
        };
        match cmd {
            Command::Start { channel, port } => {
                if channel.is_empty() || services.contains_key(&channel) {
                    continue;
                }
                let fingerprint = format!("{:x}", Sha256::digest(channel.as_bytes()));
                let nickname = format!("p{}", &fingerprint[..60]);
                let config = tor_hsservice::OnionServiceConfig::builder()
                    .nickname(nickname.parse()?)
                    .build()?;
                let (service, requests) = client
                    .launch_onion_service(config)?
                    .context("Onion service disabled")?;
                let onion = service
                    .onion_address()
                    .context("Onion identity unavailable")?
                    .display_unredacted()
                    .to_string();
                state.lock().unwrap()["onions"][&channel] = json!(onion);
                let service_stop = stop.child_token();
                let cancel = service_stop.clone();
                let slots = budget.clone();
                tokio::spawn(async move {
            ...
            Command::Revoke(channel) => {
                if let Some(token) = services.remove(&channel) {
                    token.cancel();
                }
                if let Some(onions) = state.lock().unwrap()["onions"].as_object_mut() {
                    onions.remove(&channel);
                }
            }
        }
    }
