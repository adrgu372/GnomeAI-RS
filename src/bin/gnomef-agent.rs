...
async fn async_main() -> Result<()> {
    ...
    initialize_session(&store, &session_id, &workspace, &config, &app_paths)?;
...
async fn handle_idle_op(
    ...
    match op {
        ...
        Op::MemoryClear => {
            match memory::MemoryStore::load(&core.app_paths)
                .and_then(|mut store| store.clear(&core.app_paths))
            {
...
async fn show_memory(core: &Core, events: &mpsc::Sender<Event>) {
    let config = core.config_state.read().await.clone();
    let store = match memory::MemoryStore::load(&core.app_paths) {
        Ok(store) => store,
        Err(error) => {
            recoverable_error(events, error).await;
            return;
    ...
    let mut lines = vec![format!(
        "shared memory ({}) — {} fact(s), {} conversation summarie(s), age filter: {}",
        ...
        store.facts.len(),
        store.recent_conversations.len(),
    ...
    if store.facts.len() > 50 {
        lines.push(format!("  … and {} more", store.facts.len() - 50));
    }
    lines.push("commands: /memory clear · /memory on · /memory off".into());
    notice(events, &lines.join("\n")).await;
}

/// After a finished turn, extract durable facts into the shared store —
/// the same `store/memory.json` WebTool reads and writes.
fn spawn_agent_memory_refresh(core: &Core) {
    let agent = core.agent.clone();
    let config_state = core.config_state.clone();
    let paths = core.app_paths.clone();
    tokio::spawn(async move {
        ...
        let memory_state = match memory::MemoryStore::load(&paths) {
            Ok(store) => Arc::new(RwLock::new(store)),
            Err(error) => {
                tracing::warn!("cannot load shared memory: {error}");
                return;
            }
        };
        let client = llama::LlamaClient::new();
        if let Err(error) = memory::refresh_memory_from_chat(
            &client,
            &config,
            &paths,
            &memory_state,
...
async fn run_doctor(core: &Core) -> String {
    ...
    match memory::MemoryStore::load(&core.app_paths) {
        Ok(store) => check(
            ...
            true,
            format!(
                "shared memory readable ({} facts, enabled: {})",
                store.facts.len(),
...
fn initialize_session(
    ...
    if store.live_turns(session_id)?.is_empty() {
        ...
        // Shared cross-conversation memory, same file WebTool maintains.
        if config.memory_enabled
            && let Ok(memory_store) = memory::MemoryStore::load(app_paths)
        {
            let block = memory::agent_memory_block(&memory_store, config);
            if !block.is_empty() {
...
fn policy_for(mode: SandboxMode, workspace: &Path) -> SandboxPolicy {
    match mode {
        ...
        SandboxMode::WorkspaceWrite => SandboxPolicy::workspace_write(workspace),
...
fn parse_cli() -> Result<Cli> {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "--approval" => {
                cli.approval = parse_approval(&next_value(&mut args, &arg)?)?;
            }
            "--sandbox" => {
...
fn parse_approval(value: &str) -> Result<ApprovalPolicy> {
    match value {
        ...
        _ => bail!("approval must be untrusted|on-failure|on-request|never"),
    }
}
...
fn parse_sandbox(value: &str) -> Result<SandboxMode> {
    match value {
        ...
        _ => bail!("sandbox must be read-only|workspace-write|danger-full-access"),
    }
}
