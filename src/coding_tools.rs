...
pub fn build_system_prompt(root: &Path) -> String {
    let mut s = String::from(
        "You are a coding agent working inside a single repository.\n\
         ...
         \n\
         Commands run in a sandbox with no network access and write access only\n\
         inside the workspace. A command that fails on a blocked path or a\n\
         refused connection is the sandbox working, not a bug to route around.\n\
         `web_search` and `web_fetch` are the only network-capable tools and\n\
         work only while the user-visible Web Search switch is enabled.\n",
    );
    ...
    fn spec(&self) -> ToolSpec {
        ToolSpec {
            ...
            description: "Run a shell command in the workspace. No network access. \
                          Write access is limited to the workspace directory. Use this for \
                          builds, tests and inspection — not for editing source files, which \
                          go through apply_patch."
                .into(),
            parameters: json!({
                "type": "object",
                "properties": {
