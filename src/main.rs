...
async fn web_main() -> anyhow::Result<()> {
    ...
    // WebTool polls pending approvals, while WhatsApp needs the same request
    // pushed into the conversation so the user can answer it declaratively.
    // Only the already-sanitized public request is broadcast; sudo passwords
    // remain local and never enter this channel.
    {
        let approval_state = state.clone();
        let mut approval_events = state.pending_approvals.subscribe();
        tokio::spawn(async move {
            loop {
                match approval_events.recv().await {
                    Ok(request) => notify_whatsapp_approval(&approval_state, request).await,
                    Err(tokio::sync::broadcast::error::RecvError::Lagged(skipped)) => {
                        warn!("WhatsApp approval notifier skipped {skipped} event(s)");
                    }
...
async fn api_whatsapp_inbound(
    ...
    // A pending tool call has already blocked the original model turn. Treat
    // a direct conversational answer as the out-of-band decision that resumes
    // that exact turn instead of starting a second model request. Attachments
    // never count as approval messages.
    if payload.get("media").and_then(Value::as_object).is_none() {
        if let Some(decision) = whatsapp_approval_decision(&content) {
            if let Some(reply) = handle_whatsapp_declarative_approval(
                &state,
                &cid,
                &content,
                decision,
...
enum WhatsAppApprovalDecision {
    Allow,
    Deny,
}

impl WhatsAppApprovalDecision {
...
/// Recognize a direct approval/denial statement without asking the blocked
/// model to classify its own permission. Questions, conditional wording and
/// long messages deliberately fall through to normal conversation.
fn whatsapp_approval_decision(content: &str) -> Option<WhatsAppApprovalDecision> {
    if content.chars().count() > 240 || content.contains('?') {
        return None;
    }
    let normalized = normalize_whatsapp_approval_text(content);
    ...
    if starts_with_approval_phrase(&normalized, &direct_denials) {
        return Some(WhatsAppApprovalDecision::Deny);
    }

    let words = normalized.split_whitespace().collect::<Vec<_>>();
    if words
        .iter()
        .any(|word| matches!(*word, "nu" | "not" | "never"))
        || ["doar daca", "numai daca", "only if"]
            .iter()
            .any(|phrase| normalized.contains(phrase))
        || words.iter().any(|word| {
            matches!(
                *word,
                "daca" | "dar" | "insa" | "poate" | "if" | "but" | "unless" | "maybe"
            )
        })
    {
        return None;
    ...
    if starts_with_approval_phrase(&normalized, &direct_approvals) {
        return Some(WhatsAppApprovalDecision::Allow);
    }

    for prefix in ["da ", "ok ", "yes "] {
        if let Some(rest) = normalized.strip_prefix(prefix) {
            if starts_with_approval_phrase(rest, &direct_approvals) {
                return Some(WhatsAppApprovalDecision::Allow);
            }
        }
    }
...
fn normalize_whatsapp_approval_text(content: &str) -> String {
    content
        .to_lowercase()
        .replace(['ă', 'â'], "a")
        .replace('î', "i")
...
fn starts_with_approval_phrase(content: &str, phrases: &[&str]) -> bool {
...
fn approval_id_from_text(content: &str) -> Option<String> {
    content
        .split(|ch: char| !(ch.is_ascii_alphanumeric() || ch == '_'))
        .find(|word| {
            word.len() > "approval_".len()
                && word
                    .get(.."approval_".len())
                    .is_some_and(|prefix| prefix.eq_ignore_ascii_case("approval_"))
        })
        .map(str::to_ascii_lowercase)
}

async fn handle_whatsapp_declarative_approval(
    state: &AppState,
    conversation_scope: &str,
    content: &str,
    decision: WhatsAppApprovalDecision,
    mut extra: Map<String, Value>,
) -> anyhow::Result<Option<String>> {
    let requests = state
        .pending_approvals
        .public_for_conversation(conversation_scope)
        .await;
    ...
    let standard = requests
        .iter()
        .filter(|request| request.get("kind").and_then(Value::as_str) == Some("approval"))
        .collect::<Vec<_>>();
    if standard.is_empty() {
        return Ok(Some(
            "🔑 Comanda este deja aprobată, dar sudo așteaptă autentificarea locală în WebTool. Nu trimite parola prin WhatsApp."
                .into(),
        ));
    }

    let mentioned_id = approval_id_from_text(content);
    let selected = if let Some(id) = mentioned_id.as_deref() {
        let Some(request) = standard.iter().find(|request| {
            request
                .get("id")
                .and_then(Value::as_str)
                .is_some_and(|request_id| request_id.eq_ignore_ascii_case(id))
        }) else {
            return Ok(Some(format!(
                "Nu există o aprobare activă cu ID-ul `{id}` în această conversație."
            )));
        };
        *request
        ...
        let choices = standard
            ...
            .join("\n");
        return Ok(Some(format!(
            "Sunt mai multe aprobări active. Spune `aprob ID` sau `nu aprob ID` pentru una dintre ele:\n{choices}"
        )));
    };
    ...
    // Persist the human decision before unblocking the command so even a very
    // fast tool cannot save its final response ahead of the approval message.
    extra.insert("type".into(), json!("approval_decision"));
    extra.insert("approval_id".into(), json!(id.clone()));
    extra.insert("decision".into(), json!(decision.api_value()));
    let cfg = state.config.read().await.clone();
    append_msg(
        &state.paths,
        &cfg,
    ...
    if let Err(error) = state
        ...
        .answer(
            ...
            ApprovalAnswerPayload {
                decision: decision.api_value().into(),
                credential: None,
                remember: false,
            },
        )
    ...
    {
        return Ok(Some(format!(
            "Aprobarea nu mai este activă: {error}. Comanda nu a fost executată din acest mesaj."
        )));
    }

    Ok(Some(match decision {
        WhatsAppApprovalDecision::Allow => format!(
            "✅ Aprobare înregistrată pentru {tool}: {}. Execut operația acum.",
            preview_whatsapp_text(&command, 180)
        ),
        WhatsAppApprovalDecision::Deny => format!(
            "⛔ Operația {tool} a fost respinsă: {}.",
            preview_whatsapp_text(&command, 180)
        ),
    }))
}

async fn notify_whatsapp_approval(state: &AppState, request: Value) {
    let Some(message) = format_whatsapp_approval_notice(&request) else {
        return;
    };
    let Some(scope) = request.get("scope").and_then(Value::as_str) else {
        return;
    };
    let conversation_scope = scope.split('#').next().unwrap_or(scope);
    let chat = match load_chat(&state.paths, conversation_scope) {
        Ok(chat) => chat,
        Err(error) => {
            warn!("Could not load WhatsApp chat for approval notification: {error}");
            return;
        }
    };
    let Some(chat_jid) = chat
        .extra
        .get("jid")
        .and_then(Value::as_str)
        .filter(|jid| !jid.trim().is_empty())
    else {
        warn!("WhatsApp approval scope {conversation_scope} has no saved JID");
        return;
    };
    let cfg = state.config.read().await.clone();
    send_whatsapp_message(&cfg, chat_jid, &message).await;
}

fn format_whatsapp_approval_notice(request: &Value) -> Option<String> {
    if request.get("source").and_then(Value::as_str) != Some("whatsapp") {
        return None;
    }
    let command = preview_whatsapp_text(
        request.get("command").and_then(Value::as_str).unwrap_or(""),
        1_200,
    );
    match request.get("kind").and_then(Value::as_str) {
        Some("approval") => {
            let tool = request
                ...
                .and_then(Value::as_str)
                .unwrap_or("tool");
            let reason = preview_whatsapp_text(
                request
                    .get("reason")
                    .and_then(Value::as_str)
                    .unwrap_or("Operația necesită confirmare."),
                400,
            );
            let id = request
                .get("id")
                ...
                .unwrap_or("approval");
            Some(format!(
                "🔐 Confirmare necesară pentru {tool}\n\nOperație: {command}\nMotiv: {reason}\nID: {id}\n\nRăspunde direct «aprob comanda» ca s-o execut sau «nu aprob» ca s-o respingi. Dacă ai mai multe cereri, spune «aprob {id}»."
            ))
        }
        Some("sudo_credential") => Some(format!(
            "🔑 Sudo trebuie autentificat local pentru:\n{command}\n\nIntrodu parola numai în dialogul mascat din WebTool. Nu trimite parola prin WhatsApp."
        )),
        _ => None,
    }
}

fn preview_whatsapp_text(text: &str, max_chars: usize) -> String {
...
async fn handle_whatsapp_command(state: &AppState, chat_id: &str, command: &str) -> Option<String> {
    ...
    match *cmd {
        ...
        "/sandbox" => {
            let cfg = state.config.read().await;
            Some(format!(
                "🛡️ Mod comun: {}\nÎn WhatsApp poți aproba sau respinge declarativ scrierile și comenzile. Autentificarea sudo rămâne locală în WebTool.",
                cfg.web_sandbox_mode
            ))
        }
...
mod web_security_tests {
    ...
    #[test]
    fn whatsapp_approval_intent_accepts_direct_statements_only() {
        assert_eq!(
            whatsapp_approval_decision("Da, aprob comanda."),
            Some(WhatsAppApprovalDecision::Allow)
        );
        assert_eq!(
            whatsapp_approval_decision("OK, poți executa comanda acum"),
            Some(WhatsAppApprovalDecision::Allow)
        );
        assert_eq!(
            whatsapp_approval_decision("Nu aprob comanda"),
            Some(WhatsAppApprovalDecision::Deny)
        );
        assert_eq!(
            whatsapp_approval_decision("Reject the command"),
            Some(WhatsAppApprovalDecision::Deny)
        );

        assert_eq!(
            whatsapp_approval_decision("Aprob comanda dacă testele trec"),
            None
        );
        assert_eq!(whatsapp_approval_decision("Aprob comanda?"), None);
        assert_eq!(
            whatsapp_approval_decision("Modelul a spus că aprob comanda"),
            None
        );
        assert_eq!(whatsapp_approval_decision("ok"), None);
    }

    #[test]
    fn whatsapp_approval_notice_explains_declarative_reply_and_keeps_sudo_local() {
        let standard = json!({
            "id": "approval_abc123",
            "scope": "wa_chat",
            "source": "whatsapp",
        ...
        let notice = format_whatsapp_approval_notice(&standard).unwrap();
        assert!(notice.contains("aprob comanda"));
        assert!(notice.contains("nu aprob"));
        assert!(notice.contains("approval_abc123"));
        ...
        let credential_notice = format_whatsapp_approval_notice(&credential).unwrap();
        assert!(credential_notice.contains("local"));
        assert!(credential_notice.contains("Nu trimite parola"));

        let webtool = json!({"source": "webtool", "kind": "approval"});
        assert!(format_whatsapp_approval_notice(&webtool).is_none());
    }

    #[test]
    fn whatsapp_approval_id_can_be_named_in_a_declarative_answer() {
        assert_eq!(
            approval_id_from_text("Aprob approval_A1b2C3 acum"),
            Some("approval_a1b2c3".into())
        );
        assert_eq!(approval_id_from_text("aprob comanda"), None);
    }
