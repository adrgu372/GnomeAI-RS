//! Linux sandbox for coding-agent commands.
//!
//! Three layers, applied in this order inside a freshly exec'd helper process:
...
pub enum SandboxMode {
    ...
    ReadOnly,
    /// Read the system, write only inside the workspace + scratch dirs.
    WorkspaceWrite,
    /// No sandbox at all. Requires explicit user opt-in every session.
    DangerFullAccess,
}
...
impl SandboxPolicy {
    /// Sensible default for a coding agent: can build and test, cannot reach
    /// the network, cannot touch anything outside the workspace.
    ...
    pub fn read_only(cwd: impl Into<PathBuf>) -> Self {
        let mut p = Self::workspace_write(cwd);
...
pub async fn spawn_sandboxed_with_cancel(
    ...
    // Clear the environment, then re-add only the allowlist. This is the single
    // cheapest security win here: without it every API key in the parent's env
    // is visible to whatever the model decided to run.
    cmd.env_clear();
