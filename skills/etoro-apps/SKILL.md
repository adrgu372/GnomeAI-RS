...
name: etoro-apps
version: 1.0.0
description: Enables agents to interact with the eToro API to access market data, portfolio and social features, and execute trades programmatically. Supports both OAuth SSO and manual API key authentication.
...
**OIDC endpoints:**
| Purpose | URL | Required? |
|---|---|---|
| Authorization (login) | `https://www.etoro.com/sso` | **Yes** |
| Token exchange | `https://www.etoro.com/api/sso/v1/token` | **Yes** |
| Userinfo (profile) | `https://www.etoro.com/api/sso/v1/userinfo` | No — only to fetch `sub`/`email`/`name` |
| JWKS (verify id_token) | `https://public-api.etoro.com/.well-known/jwks.json` | No — only to verify the `id_token` signature |

Where `access_token` comes from:
1. User clicks "Login with eToro" → redirects to `https://www.etoro.com/sso` with `response_type=code`, `client_id`, `redirect_uri`, `scope`, `state` (and optionally a PKCE `code_challenge` + `code_challenge_method=S256`).
2. User authenticates on eToro's side.
3. eToro redirects back to `redirect_uri` with an authorization `code` (and the same `state`).
4. The code is exchanged for tokens via `POST https://www.etoro.com/api/sso/v1/token` with `Content-Type: application/x-www-form-urlencoded`. Client credentials go **in the body** (`client_secret_post`), not in an `Authorization: Basic` header:
   - `grant_type=authorization_code`
   - `code=<auth_code>`
   - `client_id=<client_id>`
   - `client_secret=<client_secret>`
   - `redirect_uri=<callback_url>` (must exactly match the registered redirect URI)
   - `code_verifier=<pkce_verifier>` — only if a PKCE `code_challenge` was sent in step 1 (optional; the `client_secret` authenticates the confidential client)
5. Response contains:
   - `access_token` — **this is the Bearer token for API calls** (JWT)
   - `id_token` — JWT with user identity (`sub` claim = encoded user ID)
   - `token_type`: `"Bearer"`
   - `expires_in`: (varies)
   - `refresh_token`

**Mandatory vs optional for the flow.** To obtain and use an access token you only need: the **authorization endpoint**, **token endpoint**, **client_id**, **client_secret**, **redirect_uri**, and **scope**. The userinfo endpoint, JWKS URI, PKCE, and even consuming the `id_token` are all optional — add them only if you want the user's profile or verified identity.
